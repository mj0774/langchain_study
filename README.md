...
#####LangChain과 LangGraph의 1.0.x 최신 API 기준으로 실행 구조, 미들웨어, 에이전트 루프, ReAct 패턴을 중심으로 LLM 애플리케이션과 에이전트 시스템의 동작 방식을 이해하기 위해 정리한 실험 노트북 모음입니다.
#####단순 사용법이 아니라, 모델 호출 흐름, Tool 연계, 상태 관리, 출력 제어, 가드레일 적용 방식을 코드로 직접 검증하는 데 초점을 맞췄습니다.
